...
obj::obj(string path, shared_ptr<material> _mat) : mat(_mat) {
    ...
    // Convert face coordinates to true triangle objects
    triangle_faces = obj::get_triangle_faces();
}
...
bool obj::hit(const ray& r, interval ray_t, hit_record& rec) const {
    hit_record temp_rec;
    bool hit_anything = false;
    auto closest_so_far = ray_t.max;

    for (const triangle& triangle : triangle_faces) {
...
void obj::move(const vec3& moviment) {
    for (triangle& triangle : triangle_faces) {
        triangle.move(moviment);
    }
}
